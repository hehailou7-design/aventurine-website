...
export function SkinProvider({ children }: { children: ReactNode }) {
  ...
  const applySkinToDOM = (skin: SkinTheme) => {
    ...
    // 辅助：hex → rgb 数字串（如 "d4b878" → "212, 184, 120"）
    const hexToRgb = (hex: string): string => {
      const h = hex.replace('#', '')
      return `${parseInt(h.slice(0,2), 16)}, ${parseInt(h.slice(2,4), 16)}, ${parseInt(h.slice(4,6), 16)}`
    }
    
    // 设置 CSS 变量 — 这些变量在 index.css 中被引用，颜色自动切换
    root.style.setProperty('--color-primary', skin.primary)
    root.style.setProperty('--color-primary-rgb', hexToRgb(skin.primary))
    root.style.setProperty('--color-secondary', skin.secondary)
    root.style.setProperty('--color-secondary-rgb', hexToRgb(skin.secondary))
    ...
    root.style.setProperty('--color-card-border', skin.cardBorder)
    root.style.setProperty('--color-btn-text', skin.id === 'whiteLace' ? '#ffffff' : '#121212')
